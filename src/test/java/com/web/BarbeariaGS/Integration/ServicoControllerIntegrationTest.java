...
public class ServicoControllerIntegrationTest {
    ...
    @Test
    public void testAgendamentosServicos_WhenNotLoggedIn_ShouldRedirectToLogin() throws Exception {
         // Clear any existing static mocks
         Mockito.framework().clearInlineMocks();
                mockMvc.perform(get("/gerenciar/servicos"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/error/404"));
    }

    @Test
    public void testGerenciarServicos_WhenNotLoggedIn_ShouldRedirectToLogin() throws Exception {
        // Clear any existing static mocks
        Mockito.framework().clearInlineMocks();
        mockMvc.perform(get("/gerenciar/servicos/novo"))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl("/error/404"));
    }

    @Test
    public void testGerenciarNovoServico_WhenNotLoggedIn_ShouldRedirectToLogin() throws Exception {
        // Clear any existing static mocks
        Mockito.framework().clearInlineMocks();
        mockMvc.perform(get("/gerenciar/servicos/1"))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl("/error/404"));
    }

    @Test
    public void testGerenciarEditServico_WhenNotLoggedIn_ShouldRedirectToLogin() throws Exception {
        // Clear any existing static mocks
        Mockito.framework().clearInlineMocks();
        mockMvc.perform(get("/gerenciar/servicos/1/excluir"))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl("/error/404"));
    }
}
